...
2 & 0.00
  ...
  & \texttt{=C2+D2}
  & \texttt{=EXP(-A2)} \\
3 & 0.25 & 1.5243 & $-$2.0760 & 4.5728 & 2.4968 & 0.7788 \\
4 & 0.50 & 1.1723 & $-$1.3536 & 3.5169 & 2.1633 & 0.6065 \\
5 & 0.75 & 0.9162 & $-$0.8839 & 2.7487 & 1.8648 & 0.4724 \\
6 & 1.00 & 0.7253 & $-$0.5776 & 2.1759 & 1.5983 & 0.3679 \\
\bottomrule
\end{tabular}
\end{center}
...
Find $A = 1$, $B = -3$, $C = 3$.
$y(t) = e^{-t} - 3e^{-2t} + 3e^{-3t}$.
Verify by differentiating twice and substituting.
...
For $t\geq 2$: add the shifted term $[(t-2)/4 - (1/8)\sin(2(t-2))]\,u(t-2)$.
...
\textit{Hint:} $(s^2-4)Y = 1 + 2/(s^2+4)$.
Partial fractions on $1/(s^2-4)$ and $2/[(s^2-4)(s^2+4)]$.
\textbf{Answer:} $y(t) = \tfrac{1}{4}\sinh(2t) - \tfrac{1}{8}\sin(2t)$.
...
Determinant: $s^2+1$.
$X = [s^2/(s^2+1) + 1]/(s^2+1)/(s^2+1)$; solve carefully.
\textbf{Answer:} $x(t) = \cos t + t\sin t/2$, $y(t) = -t\cos t/2$ (resonant system).
...
\item The Laplace transform of $y''(t)$ given $y(0)=a$ and $y'(0)=b$ is:

(a) $s^2Y(s) + as + b$ \quad
(b) $s^2Y(s) - as - b$ \quad
(c) \textbf{(c) $s^2Y(s) - as + b$} is wrong; \quad
\textbf{(b)} $s^2Y(s)-ay(0)s - y'(0)$

\medskip
Let us restate with clean options:

(a) $sY(s)-y(0)$ \quad
(b) $s^2Y(s)+sy(0)+y'(0)$ \quad
\textbf{(c) $s^2Y(s)-sy(0)-y'(0)$} \quad
(d) $s^2Y(s)+y(0)$

\textit{Explanation:} By the Laplace derivative formula,
$\mathcal{L}\{y''\} = s^2Y - sy(0) - y'(0)$. Option \textbf{(c)} is correct.
