...
\[
  ...
  = a - b \quad(\text{finite}). \quad\text{\checkmark}
\]
...
\]
Using integration by parts ($\int \ln(1+4/u^2)\,du = u\ln(1+4/u^2) + 4\arctan(u/2)$) and evaluating from $s$ to $\infty$ gives:
\[
  \mathcal{L}\!\left\{\frac{\sin^2 t}{t^2}\right\}
  = \frac{\pi}{2} - \arctan\!\left(\frac{s}{2}\right) - \frac{s}{4}\ln\!\left(1+\frac{4}{s^2}\right)
  = \operatorname{arccot}\!\left(\frac{s}{2}\right) - \frac{s}{4}\ln\!\left(\frac{s^2+4}{s^2}\right)
    \quad\bigl(s>0\bigr).
\]
\end{infobox}
...
% -- Example 1 ----------------------------------------------------------------
\begin{infobox}{Example 1: $\mathcal{L}\!\left\{\frac{\cos(at)-\cos(bt)}{t}\right\}$}
...
\[
  ...
  = -a\cdot 0 + b\cdot 0 = 0 \quad(\text{finite}). \quad\text{\checkmark}
\]
...
% -- Example 2 ----------------------------------------------------------------
\begin{infobox}{Example 2: $\mathcal{L}\!\left\{\frac{\sin(2t)}{t}\right\}$}
...
% -- Example 3 ----------------------------------------------------------------
\begin{infobox}{Example 3: $\mathcal{L}\!\left\{\frac{e^{-t}-e^{-2t}}{t}\right\}$}
...
\midrule
0.001 & \texttt{=SIN(A2)/A2} & \texttt{=EXP(-A2)*B2} & \texttt{=0.1/2} & \texttt{0} \\
0.101 & \texttt{=SIN(A3)/A3} & \texttt{=EXP(-A3)*B3} & \texttt{=0.1/2} & \texttt{=E2+D2*(C2+C3)} \\
0.201 & \texttt{=SIN(A4)/A4} & \texttt{=EXP(-A4)*B4} & \texttt{=0.1/2} & \texttt{=E3+D3*(C3+C4)} \\
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Column C}: \texttt{=EXP(-A2)*B2} (this is the integrand
        $e^{-st}\sin(t)/t$ at $s=1$). Fill down.
  \item \textbf{Column D}: enter the trapezoid half-step weight \texttt{=0.1/2}
        in each row.
  \item \textbf{Column E} (\textbf{E2}): \texttt{0} (initial cumulative integral at $t_0$).\\
        \textbf{E3 onwards}: \texttt{=E2+D2*(C2+C3)}. This accumulates the
...
\noindent\textbf{Sample rows} (representative values):
...
\item \textbf{State the complete duality table of Topics 08--11 and explain the symmetry.}\\
  ...
  The symmetry: differentiation/integration in $t$ $\leftrightarrow$ algebraic ops in $s$;
  multiplication/division by $t$ $\leftrightarrow$ differentiation/integration in $s$.
...
\item Find $\mathcal{L}\!\left\{\dfrac{e^{-at}-e^{-bt}}{t}\right\}$ for $a,b>0$ and $s>0$.\\
  \textit{Hint:} $\mathcal{L}\{e^{-at}-e^{-bt}\} = \dfrac{1}{s+a} - \dfrac{1}{s+b}$;
  integrating from $s$ to $\infty$ gives $\ln\!\left(\dfrac{s+b}{s+a}\right)$.
...
\item Find $\mathcal{L}\!\left\{\dfrac{\cosh(at)-1}{t}\right\}$.\\
  \textit{Hint:} $\mathcal{L}\{\cosh(at)-1\} = \dfrac{s}{s^2-a^2} - \dfrac{1}{s}$.
  Existence: $(\cosh(at)-1)/t \to 0$ as $t\to 0$ (since $\cosh(0)=1$).
  Integrate from $s$ to $\infty$: $\dfrac{1}{2}\ln\!\left(\dfrac{s^2}{s^2-a^2}\right)$ (for $s>a>0$).
...
\end{enumerate}
\textit{Explanation:} Multiply by $t$ $\leftrightarrow$ differentiate; divide by $t$ $\leftrightarrow$ integrate
