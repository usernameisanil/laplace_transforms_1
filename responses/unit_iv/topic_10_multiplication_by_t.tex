...
Note: the negative sign is essential. The result tells us that if some function
$f(t)$ has Laplace transform $\ln(s^2+a^2)$, then multiplying $f(t)$ by $t$
produces a transform of $-2s/(s^2+a^2)$.
\end{infobox}
\begin{learnbox}{What Did We Learn? (Example 3)}
The multiplication-by-$t$ formula works regardless of whether we know $f(t)$
explicitly --- only $F(s)$ needs to be differentiable. Here $F(s)=\ln(s^2+a^2)$
differentiates easily by the chain rule.
\end{learnbox}
