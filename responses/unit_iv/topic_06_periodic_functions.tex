...
\]
Equivalently, multiplying numerator and denominator of the second term by
$e^{sT}$:
\[
  \boxed{
...
\texttt{t} & \texttt{f(t)} & \texttt{e\textasciicircum(-t)*f(t)} & \texttt{Cumulative Trapezoid} \\
\midrule
...
\begin{enumerate}
  ...
  \item The integral $\int_0^{10} e^{-t}f(t)\,dt$ directly approximates the Laplace transform $\mathcal{L}\{f(t)\} = \int_0^{\infty} e^{-t}f(t)\,dt = \frac{1}{s}\tanh(as/2) = \tanh(0.5) \approx 0.4621$ (for $s=1, a=1$). The periodic theorem gives $\mathcal{L}\{f(t)\} = \frac{1}{1-e^{-2}}\int_0^2 e^{-t}f(t)\,dt$, where the single-period integral is $\int_0^2 e^{-t}f(t)\,dt = (1-e^{-2})\tanh(0.5)$. Both yield the exact value $\tanh(0.5) \approx 0.4621$.
...
\item A staircase function is defined by $f(t) = n$ for $nT \leq t < (n+1)T$,
  ...
  $\mathcal{L}\{f(t)\} = \dfrac{e^{-sT}}{s(1-e^{-sT})}$.
