...
\medskip
\noindent The running sum monotonically increases and stabilizes at $\approx 0.3936$.

\begin{learnbox}{Numerical vs.\ Exact}
The exact value of the integral is $\int_0^\infty e^{-3t}\,dt = \frac{1}{3} \approx 0.3333$.  The trapezoid rule with a coarse step size $\Delta t = 0.5$ stabilizes at $\approx 0.3934$ (the trapezoid rule overestimates strictly convex decaying functions like $e^{-3t}$). Decreasing the step size $\Delta t$ (e.g., in the Python script below) brings the numerical sum arbitrarily close to the exact value $\frac{1}{3}$.  In the
...
for T in [1, 2, 5, 10, 20]:
    ...
    # Trapezoid rule (np.trapezoid in NumPy 2.0+, formerly np.trapz)
    numerical = np.trapezoid(integrand, t)
...
#     T      Numerical           Error
# ------------------------------------
#     1    0.3167376463    0.0165956871
#     2    0.3325070926    0.0008262407
#     5    0.3333332939    0.0000000395
#    10    0.3333335834    0.0000002500
#    20    0.3333343335    0.0000010002
\end{lstlisting}

\textbf{What the output shows.}  As $T$ increases from 1 to 20, the numerical
integral moves steadily closer to $\frac{1}{3}$ (with exact analytical values
$\int_0^1 e^{-3t}\,dt = \frac{1-e^{-3}}{3} \approx 0.3167376441$ and
$\int_0^2 e^{-3t}\,dt = \frac{1-e^{-6}}{3} \approx 0.3325070826$).  By $T = 5$ the value is
within $4 \times 10^{-8}$ of the asymptotic limit, confirming that $e^{-3t}$ decays fast enough
...
\begin{enumerate}[label=\textbf{P\arabic*.}, leftmargin=*, itemsep=6pt]
  ...
  \item Match each application area on the left with the transform property
        ...
        \midrule
        Damped RLC circuit response & First shifting theorem \\
        Evaluating $\int_0^\infty \frac{\sin t}{t}\,dt$ & Division by $t$ \\
        Spring-mass with initial velocity & Transform of $f'(t)$ \\
        Output of an LTI system to arbitrary input & Convolution theorem (product in $s$) \\
        \bottomrule
        \end{tabular}
        \end{center}
        (\textit{Hint:} check the roadmap in Section 6 for which topic covers
        each property; for an LTI system, $y(t) = (x * h)(t) \iff Y(s) = X(s)H(s)$.)
