...
\begin{infobox}{Example 3: Solve $y(t) = t - \int_{0}^{t} y(\tau)\,d\tau$}
...
= \int_0^t (1 - e^{-\tau})\,d\tau = [\tau + e^{-\tau}]_0^t = (t + e^{-t}) - (0 + 1)$
$= t + e^{-t} - 1$. Then:
\[
  t - \int_0^t y(\tau)\,d\tau = t - (t + e^{-t} - 1) = 1 - e^{-t} = y(t).
\]
Matches $y(t)$. $\checkmark$
\end{infobox}
...
\midrule
1   & \texttt{t} & \texttt{g(t)} & \texttt{e\textasciicircum\{-2t\}*g(t)} & \texttt{Trap.\ Sum} \\
...
$0.5$ & $0.39347$ & $0.14475$ & $0.04928$ \\
$1.0$ & $0.63212$ & $0.08556$ & $0.10891$ \\
$2.0$ & $0.86466$ & $0.01584$ & $0.15296$ \\
...
\item Find $\mathcal{L}\!\left\{\displaystyle\int_0^t \cos(2\tau)\,d\tau\right\}$.
  \hfill\textit{Hint: $\mathcal{L}\{\cos(2t)\} = \dfrac{s}{s^2+4}$; then $\dfrac{F(s)}{s} = \dfrac{1}{s^2+4}$. Answer: $\dfrac{1}{s^2+4}$.}
