...
% =============================================================
\section{\texorpdfstring{Behaviour of $F(s)$ as $s \to \infty$}{Behaviour of F(s) as s -> oo}}
% =============================================================
...
0.50  & 0.010261       & 0.105399   \\
1.00  & 0.006738       & 0.018316   \\
1.50  & 0.001867       & 0.005248   \\
2.00  & 0.000363       & 0.002479   \\
3.00  & 0.000008       & 0.002479   \\
5.00  & 0.000000       & 1.000000   \\
6.00  & 0.000000       & 403.428793 \\
\bottomrule
\end{tabular}
\end{center}

\medskip
Column B shrinks steadily toward zero --- the exponential damping factor dominates $t^3$.
Column C initially decreases on $[0, 2.5]$ (since the exponent $t^2 - 5t = t(t-5) \le 0$ for $t \in [0, 5]$), reaching a minimum of $e^{-6.25} \approx 0.00193$ at $t = 2.5$, returns to $1.0$ at $t = 5.0$, and explodes rapidly for $t > 5$ ($t = 6 \implies 403.43$, $t = 7 \implies 1.37 \times 10^5$, rapidly overflowing). Because $e^{t^2}$ violates the exponential order condition, no constant $s$ can prevent it from exploding as $t \to \infty$.
...
Column B (convergent integrand) shrinks to zero as $t$ grows, confirming the Laplace integral converges to $\mathcal{L}\{t^3\}\big|_{s=5} = 6/625 = 0.0096$.
Column C (divergent integrand) remains bounded while $t \le 5$, but explodes once $t > 5$, confirming that $\mathcal{L}\{e^{t^2}\}$ cannot exist for any $s$.
The spreadsheet provides direct numerical evidence for what the existence theorem says analytically.
\end{learnbox}
...
for T in [2, 3, 5, 7, 10]:
    try:
        ...
        print(f"  T={T:2d}: value = {val:.4e}")
    except Exception as e:
        print(f"  T={T:2d}: Integration failed -- {e}")

# Expected output:
# Attempting to integrate exp(-5t)*exp(t^2) from 0 to T:
#   T= 2: value = 2.2203e-01
#   T= 3: value = 2.2414e-01
#   T= 5: value = 4.4617e-01
#   T= 7: value = 1.3720e+05  (rapid growth begins once t > 5)
#   T=10: value = 3.4881e+20  (diverges explosively; overflow for larger T)
\end{lstlisting}
...
In contrast, while $e^{-5t} \cdot e^{t^2}$ is bounded on $[0, 5]$ because $t^2 - 5t \le 0$,
the integrand grows super-exponentially once $t > 5$, causing the integral to explode to
infinity as $T$ increases ($1.37 \times 10^5 \to 3.49 \times 10^{20} \to \infty$) and
confirming that no Laplace transform exists.
\end{learnbox}
